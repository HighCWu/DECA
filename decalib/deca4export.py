...
class DECA(nn.Module):
    ...
    # @torch.no_grad()
    def decode(self, codedict, iddict=None, return_vis=True, use_detail=True,
                render_orig=False, original_image=None, tform=None):
        images = codedict['images']
        batch_size = images.shape[0]
        
        ## decode
        verts = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'])[0]
        ...
        trans_verts0 = trans_verts[:,:,:1]
        trans_verts1 = -trans_verts[:,:,1:]
        trans_verts = torch.cat([trans_verts0, trans_verts1], 2)
        opdict = {
            'verts': verts,
        ...
        trans_verts0 = trans_verts[:,:,:2]
        trans_verts1 = trans_verts[:,:,2:] + 10
        trans_verts = torch.cat([trans_verts0, trans_verts1], 2)
        # attributes
        ...
        if return_vis:
            ...
            if render_orig and original_image is not None and tform is not None:
                ...
                opdict['images'] = original_image
                visdict['images'] = original_image
                visdict['trans_verts'] = trans_verts
            else:
                h, w = self.image_size, self.image_size
            
            visdict.update({
                'faces': self.render.faces * 1,
                'uvcoords': self.render.uvcoords * 1,
                'uvfaces': self.render.uvfaces * 1,
                'uv_face_eye_mask': self.uv_face_eye_mask * 1,
            })
    ...
    def render_images(self, codedict, opdict, visdict, use_detail=True):
        ...
        uv_gt = F.grid_sample(visdict['images'], uv_pverts.permute(0,2,3,1)[:,:,:,:2], mode='bilinear')
        if self.cfg.model.use_tex:
            ## TODO: poisson blending should give better-looking results
            uv_texture_gt = uv_gt[:,:3,:,:]*self.uv_face_eye_mask + (opdict['uv_texture'][:,:3,:,:]*(1-self.uv_face_eye_mask))
        else:
